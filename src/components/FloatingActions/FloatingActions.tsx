...
export default function FloatingActions() {
    ...
    return (
        <>
            ...
            <div className={styles.audioFloatingContainer}>
                <button
                    type="button"
                    className={`${styles.audioBtn} ${isPlaying ? styles.active : ""}`}
                    onClick={toggleAudio}
                    aria-label={isPlaying ? "إيقاف تلاوة القرآن" : "تشغيل تلاوة القرآن"}
                >
                    <div className={styles.audioPulseRing} />
                    {isPlaying ? (
                        <Volume2 size={22} className={styles.audioIcon} />
                    ) : (
                        <VolumeX size={22} className={styles.audioIcon} />
                    )}
                </button>
            </div>
